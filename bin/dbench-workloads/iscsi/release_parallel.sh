...
$DBENCH -B iscsi --iscsi-lun=$LUN --iscsi-port=$PORT --iscsi-target=$TARGET_NAME  --iscsi-portal=$PORTAL --warmup=2 -t 25 -c release_parallel_1.txt 1 &> output_1.txt &

sleep 2

echo "Running dbench instance 2"
$DBENCH -B iscsi --iscsi-lun=$LUN --iscsi-port=$PORT --iscsi-target=$TARGET_NAME  --iscsi-portal=$PORTAL --warmup=1 -t 25 -c release_parallel_2.txt 1  &> output_2.txt & 
